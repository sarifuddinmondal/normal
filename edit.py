...
# উইন্ডো সাইজ (ডেস্কটপে টেস্টিংয়ের জন্য)
...
class DraggableBtn(DragBehavior, MDFloatLayout):
    def __init__(self, btn_id, text, action, icon, color, bg, size, pos=(100, 100), **kwargs):
        ...
        self.btn = MDIconButton(
            icon=icon, icon_size=f"{28 * self.scale_val}sp", theme_icon_color="Custom",
            icon_color=get_color_from_hex(color), md_bg_color=get_color_from_hex(bg),
            pos_hint={"center_x": .5, "center_y": .5}
        )
        self.add_widget(self.btn)
        
        self.lbl = MDLabel(
            text=text, halign="center", font_style="Caption", theme_text_color="Custom",
            text_color=(1, 1, 1, 1), pos_hint={"center_x": .5, "center_y": -.35}
        )
...
# --- এডিটর স্ক্রিন ক্লাস (Android Compatible) ---
class EditScreen(Screen):
    def on_enter(self):
        ...
        # মেইন লেআউট
        self.container = MDFloatLayout(md_bg_color=(0.05, 0.05, 0.05, 1))
        
        # মোবাইল ফ্রেম সিমুলেশন
        self.mobile_x, self.mobile_y = 60, 40
        self.fw, self.fh = 340, 680
        
        self.work_area = MDFloatLayout(size_hint=(None, None), size=(self.fw, self.fh), 
                                       pos=(self.mobile_x, self.mobile_y), md_bg_color=(0, 0, 0, 1))
        with self.work_area.canvas.before:
            Color(0.15, 0.15, 0.15, 1)
            RoundedRectangle(pos=(self.mobile_x-15, self.mobile_y-15), size=(self.fw+30, self.fh+30), radius=[40,])
            Color(0, 0, 0, 1)
            RoundedRectangle(pos=self.work_area.pos, size=self.work_area.size, radius=[30,])
        
        self.container.add_widget(self.work_area)
        self.setup_sidebar()
    ...
    def setup_sidebar(self):
        ...
        self.ai_response_box = MDLabel(text="AI Ready...", theme_text_color="Custom", text_color=(0, 1, 1, 1), font_style="Caption", halign="center", size_hint_y=None, height="80dp")
        ...
        self.sidebar.add_widget(MDRaisedButton(text="SAVE & BACK", icon="content-save", on_release=self.save_and_exit, size_hint_x=1, md_bg_color=(0, 0.5, 0.2, 1)))
    ...
    def select_button(self, btn):
        if self.selected_btn:
            self.selected_btn.selected = False
    ...
    def delete_btn(self, *args):
        if self.selected_btn:
            self.work_area.remove_widget(self.selected_btn)
            self.selected_btn = None
    ...
    def save_and_exit(self, *args):
        ...
        for child in self.work_area.children:
            if isinstance(child, DraggableBtn):
                self.store.put(child.btn_id, name=child.lbl.text, action=child.action, icon=child.btn.icon, 
                               color=child.color_hex, bg=child.bg_hex, 
                               pos=(float(child.x - self.mobile_x), float(child.y - self.mobile_y)), 
                               size=child.scale_val)
        # Android-এ subprocess এর বদলে স্ক্রিন চেঞ্জ
        self.app.root.current = 'remote'
        if hasattr(self.app, 'load_ui'): self.app.load_ui()

    def load_data(self):
        for b_id in self.store.keys():
            d = self.store.get(b_id)
            btn = DraggableBtn(b_id, d['name'], d['action'], d['icon'], d['color'], d['bg'], d['size'], 
                               (d['pos'][0] + self.mobile_x, d['pos'][1] + self.mobile_y))
            self.work_area.add_widget(btn)
